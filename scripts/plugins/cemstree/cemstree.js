define(['jquery', 'zTree', 'zTree.excheck', 'zTree.exedit', 'zTree.exhide'], function($) {
	 ...
	 * idName 需获取的属性 默认是id
	 * filterFun 过滤节点的方法 此方法不传默认使用 options中的filterFun，若options中未配置，则只要选中就返回
	 * 只获取选中可使用ztree的原生方法
	 */
	$.fn.cemstreeSelected = function(idName,filterFun) {
		return $(this).cemstree('cemstreeSelected',idName,filterFun);
	};
	
	$.fn.cemstreeSelectedObj = function(filterFun) {
		return $(this).cemstree('cemstreeSelectedObj',filterFun);
	};
	
	
	$.fn.cemstreeBackinitTree= function() {
		return $(this).cemstree('cemstreeBackinitTree');
	};
	 ...
	 * node 节点数组
	 */
	$.fn.cemstreeProtoDataChange = function(node,type){
		return $(this).cemstree('cemstreeProtoDataChange',node,type);
	};
	/**
	 * 在Ztree的基础上，添加配置 使可支持批量数据时，可前端实现自动加载已经搜索的功能
	 */
	(function($) {
		$.widget("cems.cemstree", {
			...
			options: {
				...
				seachInput:false,//搜索的框的 jquery对象 
				seachRecordSelected:false,//搜索时是否记录之前的选中节点 默认是false
				maxHeight:300,//protoScrollAuto 为 true 必须指定
				onelineHeight:25,//每行的高度
				/**
				 * 默认第一次加载条数  rp 的计算为 Math.cell(maxHeight/onelineHeight)+5 计算的值必rp的大 使用计算的值
				...
				protoSeachParent:true,//查询时是否连带查询父节点 默认 不查询  为true 会降低效率
			...
			//	protoMaxAuto:true,//原型数据 是否支持自动加载  不自动加载 则加载全部   此时（protoMaxAutoExpand ，protoOneLoadGetNode protoBeginLoadRate）无效
				...
				nameOfAllNode:false,
				_initTree:{ // 用于树还原 
					_selectNode:false,// 调用 cemstreeReInitSelect 获取当前数据
					_curEle:false, // 调用cemstreeCurEle 方法获取 当前页
					_searchKey:false//调用cemstreeSeachParm  获取当前的查询条件
				}
			},
			_preCreate: function() {
				...
				if(this.options.protoScrollAuto && this.options.protoMaxAuto){
					...
					var onelineHeight = this.options.onelineHeight;
					var cRp = Math.ceil(maxHeight/onelineHeight)+5;
			...
			_creatDiv: function() {
				...
				if (!loadingDiv || loadingDiv.length <= 0) {
					...
					loadingDiv.append('<div class="cems_loading"><div>'+Msg.loading+'</div><div></div><div></div></div>');
			...
			_showLoading : function(_data){
				...
				var _py_loading = $("#"+loading).find('.cems_loading');
			...
			getData: function(callBack) {
				...
				$.http.ajax(_data.url, _param, function(data) {
					var ro = data;
					if (ro.success) {
						var nodes = ro.data || [];
						//加载数据
			...
			_dealData : function(nodes,gThis){
				...
				var _treeSet = _m._getTreeSet(_data),_setting = _m.getMSetting(gThis,_data._setting),open={open:true};
			...
			_hasInitTree : function(){
				...
				if(_data && _data._initTree){
					...
					var _treeSet = _m._getTreeSet(_data),_setting = _m.getMSetting(gThis,_data._setting),open={open:true};
					if(_init._searchKey){
						_data.seachInput && _data.seachInput.val(_init._searchKey);
			...
			cemstreeSelectedObj:function(filterFun){
				var _this = $(this.element);
				var _data = this.options;
				var treeObj = $.fn.zTree.getZTreeObj(_data.id);
				var idArr = [];
				...
				if(chkStyle && chkStyle == "checkbox" && _data.protoMaxAutoExpand >= 0 
					...
					var seletedRecored = _data.SelectedRecord.values();
					var nodes = treeObj.transformTozTreeNodes($.extend(true,[],seletedRecored));
			 ...
			 * 获取选中 
			 */
			cemstreeSelected: function(idName,filterFun) {
				var _this = $(this.element);
				var _data = this.options;
				var treeObj = $.fn.zTree.getZTreeObj(_data.id);
				var idArr = [];
			...
			cemstreeProtoDataChange:function(nodes,type){
				if(!nodes || nodes.length<0){
					return;
				}
				var gThis = this;
			...
			cemstreeBackinitTree:function(){
				var _init = {};
				_init._searchKey = this.cemstreeSeachParm();
				_init._selectNode = this.cemstreeReInitSelect();
				_init._curEle = this.cemstreeCurEle();
				return $.extend(true,{},_init);
			},
			/**
			 * 获取当前的查询条件
			 */
			cemstreeSeachParm:function(){
				var _data = this.options;
...
//				_m._reGetSelectedRecord(this);//获取选中 并记录
		...
		var _m = {
			...
			_protoQueryFun : function(seachVal,gThis){
				var _data = gThis.options;
				var _treeSet = _m._getTreeSet(_data);
				var data = _data.prototypeData,reg,protoSeachParent =_data.protoSeachParent,
